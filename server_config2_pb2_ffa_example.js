class sdServerConfig
{
	...
	static game_title = 'Star Defenders: PvP arena';
	...
	static onExtraWorldLogic( GSPEED )
	{
		...
		for ( let i = 0; i < sdWorld.sockets.length; i++ )
		{
			...
			{
				if ( sdWorld.sockets[ i ].character.y + sdWorld.sockets[ i ].character.hitbox_y2 > sdWorld.world_bounds.y2 - 8 )
	...
	static PlayerSpawnPointSeeker( character_entity, socket )
	{
		...
		do
		{
			...
			if ( tr > max_tr * 0.4 || sdWorld.last_hit_entity === null || ( sdWorld.last_hit_entity.GetClass() === 'sdBlock' && sdWorld.last_hit_entity.material !== sdBlock.MATERIAL_SHARP ) ) // Only spawn on ground
			{
				...
				{
					if ( x > sdWorld.push_fields[ i ].x )
					if ( x < sdWorld.push_fields[ i ].x + sdWorld.push_fields[ i ].w )
					if ( y > sdWorld.push_fields[ i ].y )
					if ( y < sdWorld.push_fields[ i ].y + sdWorld.push_fields[ i ].h )
					{
						bad_areas_near = true;
						break;
					}
