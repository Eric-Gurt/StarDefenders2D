...
class sdSetrDestroyer extends sdEntity
{
	...
	CanAttackEnt( ent )
	{
		if ( ( ent === this._current_target && ent._ai_team !== this._ai_team ) || ent.build_tool_level > 19 )
		return true;
		else
		{
			if ( ( ( ent.build_tool_level >= 20 && ent._ai_team === 0 ) && ent._ai_team !== this._ai_team ) || ( ent._ai_enabled !== sdCharacter.AI_MODEL_NONE && ent._ai_team !== this._ai_team ) )
			{
		...
		else
		{
			...
			if ( sdWorld.is_server )
			{
				if ( this._attack_timer <= 0 )
				{
					...
					let targets_raw = sdWorld.GetAnythingNear( this.x, this.y, sdSetrDestroyer.attack_range, null, [ 'sdCharacter', 'sdPlayerDrone', 'sdPlayerOverlord', 'sdTurret', 'sdManualTurret', 'sdCube', 'sdDrone', 'sdCommandCentre', 'sdEnemyMech', 'sdOverlord', 'sdSpider' ] );
