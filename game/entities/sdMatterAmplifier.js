...
class sdMatterAmplifier extends sdEntity
{
	...
	constructor( params )
	{
		...
		this.multiplier = params.multiplier || 0; // Crystal regeneration multiplier, used for higher tier matter amplifiers
		this._hmax = 320 * ( 1 + ( 0.5 * this.multiplier ) );
	...
	{
		this.matter = Math.min( this.matter_max, this.matter + GSPEED * 0.001 * this.matter_max / 80 * ( sdMatterAmplifier.relative_regen_amplification_to_crystals * ( 1 + this.multiplier ) ) );
	...
	//	return 0; // Hack
		...
		if ( this.multiplier === 7 ) // Needs cube shards matter upgrades to be placable
