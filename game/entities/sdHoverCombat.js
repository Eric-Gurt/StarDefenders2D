...
class sdHoverCombat extends sdEntity
{
	static init_class()
	{
		sdHoverCombat.img_hover_combat_shell = sdWorld.CreateImageFromFile( 'Hover_Combat_Shell' );
		sdHoverCombat.img_hover_engine= sdWorld.CreateImageFromFile('Hover_Engine');
		sdHoverCombat.img_hover_combat_canopy= sdWorld.CreateImageFromFile('Hover_Combat_Canopy');
		sdHoverCombat.img_hover_combat_boost = sdWorld.CreateImageFromFile( 'Hover_Combat_Boost' );
		sdHoverCombat.img_hover_combat_broken = sdWorld.CreateImageFromFile( 'Hover_Combat_Broken' );
		
		sdHoverCombat.img_hover_mg = sdWorld.CreateImageFromFile( 'hover_mg' );
		sdHoverCombat.img_hover_rl = sdWorld.CreateImageFromFile( 'hover_turret' );
		
		sdHoverCombat.driver_slots = 3;
		
		sdHoverCombat.slot_hints = [
			'Entered slot 1: Driver',
			'Entered slot 2: Machinegunner',
			'Entered slot 3: Rocket turret operator',
		];
	...
	AddDriver( c )
	{
		...
		for ( var i = 0; i < sdHoverCombat.driver_slots; i++ )
		//for ( var i = 2; i < sdHoverCombat.driver_slots; i++ ) // Hack
		{
		...
		if ( best_slot >= 0 )
		{
			...
			if ( c._socket )
			c._socket.emit('SERVICE_MESSAGE', sdHoverCombat.slot_hints[ best_slot ] );
	...
	ExcludeDriver( c )
	{
		...
		for ( var i = 0; i < sdHoverCombat.driver_slots; i++ )
		{
			if ( this[ 'driver' + i ] === c )
			{
				...
				c.x = this.x + ( i / ( sdHoverCombat.driver_slots - 1 ) ) * ( this.hitbox_x2 - this.hitbox_x1 );
	...
	DrawHUD( ctx, attached ) // foreground layer
	{
		...
		sdEntity.Tooltip( ctx, "Combat Hover" );
	...
	Draw( ctx, attached )
	{
		...
		for ( var i = 0; i < sdHoverCombat.driver_slots; i++ )
		{
			if ( this[ 'driver' + i ] )
			{
				ctx.save();
				{
					...
					ctx.scale( -0.8, 0.8 );
					ctx.translate( ( -32 + ( 1 - i / ( sdHoverCombat.driver_slots - 1 ) ) * 64 ) * 0.5, 3 );
		...
		if ( this.hea > 0 )
		{
			ctx.drawImageFilterCache( this.driver0 ? sdHoverCombat.img_hover_combat_shell : sdHoverCombat.img_hover_combat_canopy, - 32, - 16, 64,32 );
			ctx.drawImageFilterCache( this.driver0 ? sdHoverCombat.img_hover_combat_canopy : sdHoverCombat.img_hover_combat_shell, - 32, - 16, 64,32 );
	        var i = 0;
			...
			if ( this[ 'driver' + i ] )
			{
				...
				ctx.drawImageFilterCache( sdHoverCombat.img_hover_mg, - 16, - 16, 32,32 );
			...
			if ( this[ 'driver' + i ] )
			{
				...
				ctx.drawImageFilterCache( sdHoverCombat.img_hover_rl, - 16, - 16, 32,32 );

				ctx.restore();
			}
		}
		else
		ctx.drawImageFilterCache( sdHoverCombat.img_hover_combat_broken, - 32, - 16, 64,32 );
