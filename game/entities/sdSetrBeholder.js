...
class sdSetrBeholder extends sdEntity
{
	static init_class()
	{

		sdSetrBeholder.img_vehicle = sdWorld.CreateImageFromFile( 'sdSetrBeholder' );
		sdSetrBeholder.img_weapon1 = sdWorld.CreateImageFromFile( 'setr_lmg' );

		
		sdSetrBeholder.slot_hints = [
			'Entered slot 1: Driver, gunner' // sdSetrBeholder
		];
		
		// Maybe add plasma shotgun and repulsor variants?
		//sdSetrBeholder.TYPE_LMG = 0;
		//sdSetrBeholder.TYPE_SHOTGUN = 0;
	...
	get hitbox_y1() { return this.driver0? -14 : 12; }
	get hitbox_y2() { return 30; }
	...
	constructor( params )
	{
		...
		this._ai_team = 7; // 7 is AI team for Setr
	...
	{
		if ( this.hea > 0 && this.driver0 )
		{
			let di_to_shield = sdWorld.Dist2D( x, y, this.x, this.y + this.hitbox_y1 + 4 );

			if ( di_to_shield < 22 ) // Shields are protecting the cabin of the vehicle
			{
				sdSound.PlaySound({ name:'shield', x:this.x, y:this.y, volume:1 });
				return 1/3;
			}
		}
	    // Bottom part of the vehicle takes full damage
		return 1;
	}
	IsVehicle()
	{
	...
	onThink( GSPEED ) // Class-specific, if needed
	{
		...
		if ( this.driver0 && this.hea > 0 )
		{
			...
			if ( this.matter >= cost )
			{
				...
				if ( ( this.sx > -speed_cap || x_force > 0 ) || ( this.sx < speed_cap || x_force < 0 ) ) // Cap horizontal speed
		...
		if ( sdWorld.is_server && this.hea > 0 )
		{
			...
			if ( this.driver0 && this._bullets_reload <= 0 )
			{
				...
				{
					sdSound.PlaySound({ name: 'gun_pistol', x:this.x, y:this.y,volume:0.8, pitch: 1.2 });
					sdSound.PlaySound({ name:'enemy_mech_attack4', x:this.x, y:this.y, volume:1.25, pitch: 0.7 });
					// Attack fires 1 projectile.
					let gun_y =  this._bullets % 2 === 0 ? 14 : 22; // Determines which gun shoots.
					let gun_x =  this._tilt > 0 ? 6 : -6;
					let bullet_obj = new sdBullet({ x: this.x + gun_x, y: this.y + gun_y });
				...
				{
					sdSound.PlaySound({ name: 'supercharge_combined2', x:this.x, y:this.y, volume: 1, pitch: 1.5 });
					sdSound.PlaySound({ name: 'enemy_mech_charge', x:this.x, y:this.y, volume: 1.5, pitch: 1.2 });
					this._bullets = 50;
	...
	Draw( ctx, attached )
	{
		...
		let image = sdSetrBeholder.img_vehicle;
		...
		//if ( this.type !== sdSetrBeholder.TYPE_BIKE )
		DrawDrivers();
		...
		xx = 2;
		ctx.drawImageFilterCache( image, xx * width, 0, width, 64, - width/2, -32, width, 64 );
		if ( this.driver0 && this.hea > 0 ) // Is someone inside?
		{
			xx = 1; // Draw the shield
			ctx.drawImageFilterCache( image, xx * width, 0, width, 64, - width/2, -32, width, 64 );
		}
		...
		if ( this.guns )
		{
			var i;
				...
				if ( this[ 'driver' + i ])
				{
					...
					ctx.drawImageFilterCache( sdSetrBeholder.img_weapon1, - 16, - 16, 32,32 );
...
//sdSetrBeholder.init_class();

export default sdSetrBeholder;
