...
class sdFighterHover extends sdEntity
{
	static init_class()
	{
		sdFighterHover.img_hover = sdWorld.CreateImageFromFile( 'f_hover' );
		sdFighterHover.img_hover_boost = sdWorld.CreateImageFromFile( 'f_hover_boost' );
		sdFighterHover.img_hover_broken = sdWorld.CreateImageFromFile( 'f_hover_broken' );
		...
		sdFighterHover.slot_hints = [
			...
			'Entered slot 2: Railgun operator',
			'Entered slot 3: Rocket launcher operator',
			'Entered slot 4: Passenger',
		];
		
		sdWorld.entity_classes[ this.name ] = this;
	}
	...
	AddDriver( c )
	{
		...
		for ( var i = 0; i < sdFighterHover.driver_slots; i++ )
		{
		...
		if ( best_slot >= 0 )
		{
			...
			if ( c._socket )
			c._socket.SDServiceMessage( sdFighterHover.slot_hints[ best_slot ] );
	...
	ExcludeDriver( c )
	{
		...
		for ( var i = 0; i < sdFighterHover.driver_slots; i++ )
		{
			if ( this[ 'driver' + i ] === c )
			{
				...
				c.x = this.x + ( i / ( sdFighterHover.driver_slots - 1 ) ) * ( this._hitbox_x2 - this._hitbox_x1 );
	...
	onThink( GSPEED )
	{
		...
		if ( sdWorld.is_server && this.hea > 0 )
		{
			...
			if ( this._bullets_reload <= 0 )
			{
				...
				{
					sdSound.PlaySound({ name:'cube_attack', x:this.x, y:this.y, volume:0.5, pitch:1.5 });
				...
				if ( this._bullets <= 0 || ( this._bullets < 500 && this.driver1._key_states.GetKey( 'KeyR' ) ) )
				{
	...
	Draw( ctx, attached )
	{
		...
		for ( var i = 0; i < sdFighterHover.driver_slots; i++ )
		{
			if ( this[ 'driver' + i ] )
			{
				ctx.save();
				{
					...
					ctx.scale( -0.8, 0.8 );
					ctx.translate( ( -32 + ( 1 - i / ( sdFighterHover.driver_slots - 1 ) ) * 64 ) * 0.5, 3 );
		...
		if ( this.hea > 0 )
		{
			ctx.drawImageFilterCache( this.driver0 ? sdFighterHover.img_hover_boost : sdFighterHover.img_hover, - 32, - 16, 64,32 );
			...
			if ( this[ 'driver' + i ] )
			{
				...
				ctx.drawImageFilterCache( sdFighterHover.img_hover_mg, - 16, - 16, 32,32 );
			...
			if ( this[ 'driver' + i ] )
			{
				...
				ctx.drawImageFilterCache( sdFighterHover.img_hover_rl, - 16, - 16, 32,32 );

				ctx.restore();
			}
		}
		else
		ctx.drawImageFilterCache( sdFighterHover.img_hover_broken, - 32, - 16, 64,32 );
