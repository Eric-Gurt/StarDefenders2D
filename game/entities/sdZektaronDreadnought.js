			...
			if ( sdWorld.is_server )
			{
				if ( this._attack_timer <= 0 )
				{
					...
					if ( array_of_enemies.indexOf( targets_raw[ i ].GetClass() ) !== -1 )
					{
						...
						if ( sdWorld.CheckLineOfSight( this.x, this.y, targets_raw[ i ].x, targets_raw[ i ].y, targets_raw[ i ], [ 'sdZektaronDreadnought' ], [ 'sdBlock', 'sdDoor', 'sdMatterContainer', 'sdMatterAmplifier' ] ) )
						...
						else
						{
							...
							if ( targets_raw[ i ].GetClass() === 'sdCharacter' && targets_raw[ i ]._ai_team !== this._ai_team && this.CanAttackEnt( targets_raw[ i ] ) ) // Highly wanted by sdZektaronDreadnoughts in this case
							{
								targets.push( targets_raw[ i ] );
							}
						}
