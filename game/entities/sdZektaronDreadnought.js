...
class sdZektaronDreadnought extends sdEntity
{
	static init_class()
	{
		sdZektaronDreadnought.img_dreadnought = sdWorld.CreateImageFromFile( 'zektaron_dreadnought' );
		sdZektaronDreadnought.img_glow = sdWorld.CreateImageFromFile( 'hit_glow' );
		
		sdZektaronDreadnought.dreadnought_counter = 0;
		
		sdZektaronDreadnought.death_duration = 30;
		sdZektaronDreadnought.post_death_ttl = 120;
	...
	FireFocusBeam() // Fire 4 projectiles, up, left, down and right or diagonally, depends on parameter
	{
		if ( !sdWorld.is_server )
		return;

		let bullet_obj1 = new sdBullet({ x: this.x - 10, y: this.y + 50 });
					...
					bullet_obj1._rail_circled = true;
		...
		let bullet_obj2 = new sdBullet({ x: this.x, y: this.y + 55 });
					...
					bullet_obj2.explosion_radius = 4; 
					bullet_obj2._rail = true;
					bullet_obj2._rail_circled = true;
		...
		let bullet_obj3 = new sdBullet({ x: this.x + 10, y: this.y + 50 });
					...
					bullet_obj3.explosion_radius = 4; 
					bullet_obj3._rail = true;
					bullet_obj3._rail_circled = true;
	...
	CanAttackEnt( ent )
	{
		if ( ( ent === this._current_target && ent._ai_team !== this._ai_team ) || ent.build_tool_level > 24 )
		return true;
		else
		{
			if ( ( ( ent.build_tool_level >= 25 && ent._ai_team === 0 ) && ent._ai_team !== this._ai_team ) || ( ent._ai_enabled !== sdCharacter.AI_MODEL_NONE && ent._ai_team !== this._ai_team ) )
			{
	...
	GetRandomEntityNearby() // Scans random area on map for potential entities
	{
		...
		if ( [ 'sdCharacter', 'sdPlayerDrone', 'sdPlayerOverlord', 'sdTurret', 'sdManualTurret', 'sdCube', 'sdDrone', 'sdCommandCentre', 'sdEnemyMech', 'sdOverlord', 'sdSpider', 'sdShurgTurret', 'sdTzyrgAbsorber', 'sdVeloxMiner', 'sdShurgExcavator', 'sdSetrDestroyer' ].indexOf( e.GetClass() ) !== -1 )
		if ( e.IsVisible( this ) )
	...
	GetHitDamageMultiplier( x, y ) // Weakpoint logic
	{
		if ( this.hea > 0 )
		{
			let di_to_weakpoint = sdWorld.Dist2D( x, y, this.x, this.y + 48 );
			// let di_to_body = sdWorld.Dist2D( x, y, this.x, this.y );

			if ( di_to_weakpoint < 24 )
			return 3;
		}
	...
	Damage( dmg, initiator=null )
	{
		...
		if ( initiator )
		{
			...
			if ( initiator.GetClass() !== 'sdZektaronDreadnought' )
		...
		if ( this.hea > 0 )
		{
			if ( Math.ceil( this.hea / this._hmax * 8 ) !== Math.ceil( old_hp / this._hmax * 8 ) )
			{
				...
				//if ( sdWorld.last_hit_entity === null || ( sdWorld.last_hit_entity.GetClass() === 'sdBlock' && sdWorld.last_hit_entity.material === sdBlock.MATERIAL_GROUND ) )
				{
					...
					sdWorld.SendEffect({ x:drone.x, y:drone.y, type:sdEffect.TYPE_TELEPORT, filter:'hue-rotate(140deg)' });
				...
				//if ( sdWorld.last_hit_entity === null || ( sdWorld.last_hit_entity.GetClass() === 'sdBlock' && sdWorld.last_hit_entity.material === sdBlock.MATERIAL_GROUND ) )
				{
					sdSound.PlaySound({ name:'teleport', x:drone2.x, y:drone2.y, volume:0.5 });
					sdWorld.SendEffect({ x:drone2.x, y:drone2.y, type:sdEffect.TYPE_TELEPORT, filter:'hue-rotate(140deg)' });
		...
		{	
			...
			// sdWorld.SpawnGib( this.x + 30, this.y + 11, this.sx  + Math.random() * 1, this.sy + Math.random() * 1 , 2, sdGib.CLASS_SETR_DESTROYER_PARTS , null, null, 100, this, 2 );
	...
	onThink( GSPEED ) // Class-specific, if needed
	{
		...
		else
		{
			...
			if ( sdWorld.is_server )
			{
				...
				if ( this._move_dir_timer <= 0 )
				{
					...
					if ( closest )
					{
						...
						if ( closest_di_real < sdZektaronDreadnought.attack_range ) // close enough to dodge obstacles
						{
							let an = Math.random() * Math.PI * 2;
							...
							{
								for ( let ideas = Math.max( 5, 40 / sdZektaronDreadnought.dreadnought_counter ); ideas > 0; ideas-- )
								{
			...
			if ( 
					...
					sdWorld.CheckLineOfSight( this.x, this.y, this.x + this._move_dir_x * 224, this.y + this._move_dir_y * 128, this, sdCom.com_visibility_ignored_classes, null ) &&  // Can move forward
				( 
				   !sdWorld.CheckLineOfSight( this.x, this.y, this.x + this._move_dir_x * 400, this.y + this._move_dir_y * 400, this, sdCom.com_visibility_ignored_classes, null ) || // something is in front in distance
				   !sdWorld.CheckLineOfSight( this.x, this.y, this.x - this._move_dir_x * 450, this.y - this._move_dir_y * 450, this, sdCom.com_visibility_ignored_classes, null ) || // allow retreat from wall behind
				   !sdWorld.CheckLineOfSight( this.x, this.y, this.x - this._move_dir_y * 168, this.y + this._move_dir_x * 168, this, sdCom.com_visibility_ignored_classes, null ) || // side
				   !sdWorld.CheckLineOfSight( this.x, this.y, this.x + this._move_dir_y * 168, this.y - this._move_dir_x * 168, this, sdCom.com_visibility_ignored_classes, null ) // side
				   ) )
			{
			...
			if ( sdWorld.is_server )
			{
				if ( this._attack_timer <= 0 )
				{
					...
					if ( this._focusbeam_attack_timer <= 0 )
					{
						if ( this.hea < ( this._hmax / 2 ) )
						{
							this.FireFocusBeam();
							this._focusbeam_attack_timer = 4.5;
						}
					}
					...
					let targets_raw = sdWorld.GetAnythingNear( this.x, this.y, sdZektaronDreadnought.attack_range, null, [ 'sdCharacter', 'sdPlayerDrone', 'sdPlayerOverlord', 'sdTurret', 'sdManualTurret', 'sdCube', 'sdDrone', 'sdCommandCentre', 'sdEnemyMech', 'sdOverlord', 'sdSpider', 'sdShurgTurret', 'sdTzyrgAbsorber', 'sdSetrDestroyer' ] );
					...
					if ( ( targets_raw[ i ].GetClass() === 'sdCharacter' && targets_raw[ i ]._ai_team !== this._ai_team && targets_raw[ i ].hea > 0 && this.CanAttackEnt( targets_raw[ i ] )  ) ||
						...
						( targets_raw[ i ].GetClass() === 'sdCommandCentre' ) ||
						 ...
						 ( targets_raw[ i ].GetClass() === 'sdCube' && this.hea < ( this._hmax / 2 ) ) ||
						 ( targets_raw[ i ].GetClass() === 'sdDrone' && targets_raw[ i ]._ai_team !== this._ai_team && targets_raw[ i ]._hea > 0 ) )
					{
						if ( sdWorld.CheckLineOfSight( this.x, this.y, targets_raw[ i ].x, targets_raw[ i ].y, targets_raw[ i ], [ 'sdZektaronDreadnought' ], [ 'sdBlock', 'sdDoor', 'sdMatterContainer' ] ) )
						...
						{
							if ( this.hea < ( this._hmax / 2 ) )
							if ( targets_raw[ i ].GetClass() === 'sdCharacter' && targets_raw[ i ]._ai_team !== this._ai_team && this.CanAttackEnt( targets_raw[ i ] ) ) // Highly wanted by sdDreadnoughts in this case
							{
								targets.push( targets_raw[ i ] );
							}
						}
					...
					for ( let i = 0; i < targets.length; i++ )
					{
						...
						if ( this._lasers <= 0 && ( this.hea > this._hmax / 1.5 ) ) // Greater than 75% HP
						{
							this._lasers = 7;
							this._lasers_attack_timer = 60;
						}
						else
						if ( this._lasers <= 0 )
						{
							this._lasers = 13;
							this._lasers_attack_timer = 75;
						}

						sdSound.PlaySound({ name:'alien_laser2', x:this.x, y:this.y, volume:0.9, pitch: 0.9 });

						break;
					}

					if ( targets.length === 0 ) // lower seek rate when no targets around
					this._attack_timer = 60 + Math.random() * 10;
					else
					{
				...
				else
				{
					...
					if ( this._lasers_attack_timer > 0)
					this._lasers_attack_timer -= GSPEED;
					if ( this._focusbeam_attack_timer > 0)
					this._focusbeam_attack_timer -= GSPEED;
					if ( this._deploy_minions_timer > 0)
					this._deploy_minions_timer -= GSPEED;
	...
	Draw( ctx, attached )
	{
		...
		{
			ctx.drawImageFilterCache( sdZektaronDreadnought.img_dreadnought, xx * 224, 0, 224, 128, - 112 / 3, - 48 / 1.5, 112 / 1.5, 64 );
		}
		else
		if ( this.hea > 0 )
		{
...
export default sdZektaronDreadnought;
