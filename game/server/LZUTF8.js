...
// Override LZW methods
{
	const mode = { outputEncoding: 'StorageBinaryString' };
	const mode2 = { inputEncoding: 'StorageBinaryString' };
	LZW.lzw_encode = ( s )=>
	{
		return LZUTF8.compress( s, mode );
	};
	LZW.lzw_decode = ( s )=>
	{
		return LZUTF8.decompress( s, mode2 );
	};
...
/////////////
	...
	/*let stats = [ 0, 0, '', '' ]; [ 164.4009004831314, 750.033798456192 ], old way was just simply faster
	
	LZW.lzw_encode_csapi = async function compressAndEncode(inputString) {
		
		let t1 = performance.now();
		
		let r = LZW.lzw_encode( inputString );
		...
		while ((result = await reader.read())) {
			if (result.done) {
				...
				//const encodedData = (btoa(String.fromCharCode(...compressedData)));
	...
	LZW.lzw_decode_csapi = async function decompressAndDecode(encodedString) {
	...
	LZW.lzw_encode_csapi = async function( s )
	{
		return await LZW.lzw_encode( s );
	};
	LZW.lzw_decode_csapi = async function( s )
	{
		return await LZW.lzw_decode( s );
	};
}
